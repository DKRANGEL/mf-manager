...
function gerarPedidoMFHTML(pedido) {
    ...
    const LABEL_MAP = { cidade:'CIDADE', evento:'EVENTO', local:'LOCAL', condicao:'COND. PAGAMENTO' };
...
function _mfGerarSecaoHTML(sec) {
    ...
    const rows = (sec.itens || []).map((item, i) => {
        ...
        if (cols?.imagem) {
            tds += `<td style="${tdC}">${item.imagem ? `<img src="/data/produtos/${esc(item.imagem)}" style="width:36px;height:36px;object-fit:contain;border-radius:3px;">` : ''}</td>`;
        }
